...
class ChipsInput<T> extends StatefulWidget {
  ...
  final void Function()? onChangedTag;
...
class ChipsInputState<T> extends State<ChipsInput<T>>
  ...
  void performAction(TextInputAction action) {
    switch (action) {
      case TextInputAction.done:
        if (widget.separator == '\n') {
          final String newTag = _value.normalCharactersText;
          if (newTag.isEmpty) {
            _updateTextInput();
            return;
          }
  ...
  void updateEditingValue(TextEditingValue value) {
    ...
    final int index = widget.chipTextValidator(value.text);
    if (index == -1) {}

    final newTextEditingValue = value;
    ...
    final String tagText = _value.normalCharactersText;
    if (tagText.isNotEmpty) {
      final String lastString = tagText.substring(tagText.length - 1);
      ...
      if (lastString == widget.separator) {
        final String newTag = tagText.substring(0, tagText.length - 1);
        if (newTag.isEmpty) {
          _updateTextInput();
          return;
        }
  ...
  void addChip(T data) {
    final String enteredText = _value.normalCharactersText;
  ...
  @override
  Widget build(BuildContext context) {
    final List<Widget> chipsChildren = _chips
        .map<Widget>((data) => widget.chipBuilder!(context, this, data))
        .toList();
    final Widget chipsBox = ConstrainedBox(
